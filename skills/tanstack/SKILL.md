...
Headless, type-safe form library.
Complete reference: `references/tanstack-form.md`
...
const form = useForm({
  ...
  validators: { onChange: zodSchema },  // v1+: no adapter needed
  onSubmit: async ({ value, formApi }) => { await saveUser(value) },
})
...
Key patterns: sync/async validators, `asyncDebounceMs`, `form.Subscribe` for submit state, array fields (`pushValue`/`removeValue`), linked fields via `listeners`, `asyncDefaultValues` for edit mode.
...
- TanStack AI: `references/tanstack-ai.md`
