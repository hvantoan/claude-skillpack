...
|------|------|--------|----------|---------|
| **1: Technical Infrastructure** | `DbContext`, logging, auth, Result pattern, middleware, DI extensions | ✅ Freely | `Shared/`, `Infrastructure/` | `AppDbContext`, `GlobalExceptionHandlerMiddleware`, `IEndpoint` discovery |
...
| **3: Feature-Specific Logic** | Validation, queries, commands, DTOs per use case | ❌ Never | `Features/{Domain}/{Action}/` | `CreateOrderValidator`, `GetOrderByIdEndpoint` |
...
public static async Task<IResult> Handle(
    ...
    if (req.Items.Count == 0)
        return Results.BadRequest(new { success = false, message = "Order must have items" });
    var total = req.Items.Sum(i => i.Price * i.Quantity);
    if (total <= 0)
        return Results.BadRequest(new { success = false, message = "Total must be positive" });
    var order = new Order {
...
│   ├── Money.cs
│   └── OrderStatus.cs
├── Contracts/             ← Tier 1: External API interfaces
│   └── IPartnerClient.cs
├── Extensions/            ← Tier 1: Infrastructure wiring
│   └── EndpointExtensions.cs
└── Middleware/            ← Tier 1: Cross-cutting concerns
...
- [ ] **Is this infrastructure?** (DbContext, auth, logging) → Tier 1, share freely
- [ ] **Is this a domain concept with behavior?** (entity, value object) → Tier 2, share carefully
- [ ] **Is this feature-specific?** (validation, query, DTO) → Tier 3, keep in feature slice
...
```
src/
├── MyApp.Api/             ← endpoints, DI, middleware
